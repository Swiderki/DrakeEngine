...
export class Overlap {
  ...
  isHappening(): boolean {
    ...
    const obj1AABB = [
      { x: Math.min(pos1.x + box1[0].x, pos1.x + box1[1].x), y: Math.min(pos1.y + box1[0].y, pos1.y + box1[1].y), z: Math.min(pos1.z + box1[0].z, pos1.z + box1[1].z) },
      { x: Math.max(pos1.x + box1[0].x, pos1.x + box1[1].x), y: Math.max(pos1.y + box1[0].y, pos1.y + box1[1].y), z: Math.max(pos1.z + box1[0].z, pos1.z + box1[1].z) },
    ];
  
    const obj2AABB = [
      { x: Math.min(pos2.x + box2[0].x, pos2.x + box2[1].x), y: Math.min(pos2.y + box2[0].y, pos2.y + box2[1].y), z: Math.min(pos2.z + box2[0].z, pos2.z + box2[1].z) },
      { x: Math.max(pos2.x + box2[0].x, pos2.x + box2[1].x), y: Math.max(pos2.y + box2[0].y, pos2.y + box2[1].y), z: Math.max(pos2.z + box2[0].z, pos2.z + box2[1].z) },
    ];
